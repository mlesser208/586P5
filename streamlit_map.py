...
def create_map(df_valid: pd.DataFrame):
    ...
    for cluster_key, meta in clusters.items():
        ...
        if cluster_points[cluster_key]:
            ...
            # Track for toggling (store layer names so they can be referenced in JS)
            cluster_layer_pairs.append(
                (overview_cluster.get_name(), detail_cluster.get_name())
            )
    ...
    if cluster_layer_pairs:
        layer_pairs_js = ",\n                ".join(
            [f"{{overview: {overview}, detail: {detail}}}" for overview, detail in cluster_layer_pairs]
        )
