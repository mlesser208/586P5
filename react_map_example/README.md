...
- **Throttled callbacks:** Viewport change callbacks fire on `moveend/zoomend` with `requestAnimationFrame`, so your React state updates only after a pan or zoom settles (not every animation frame), and duplicate viewport values are skipped to avoid state churn.
- **Stable options:** Initial `center`/`zoom` are captured once so parent re-renders do not reset the camera, and the Mapbox
  instance is created in an empty-deps effect so it will not be torn down and rebuilt as props change.
